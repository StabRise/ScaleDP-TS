...
[`PaddleRecognizer`](/docs/stages/recognise/paddle-recognizer) is the same idea
with PP-OCR doing the reading.
...
`'word'` is the default: Tesseract's own word boxes, mapped back through the crop
...
`'region'` restores ScaleDP's behaviour: one box per region the detector found,
carrying everything read inside it. Since the detectors here are line-level, so
are those boxes.

[`PaddleRecognizer`](/docs/stages/recognise/paddle-recognizer) takes the same
parameter with the same default, so swapping the engine does not change how
finely the page comes back.
...
| `boxLevel` | `'word'` \| `'region'` | `'word'` | Word boxes are Tesseract’s own. Pick “region” to get one box per region the detector found instead, which is what Python ScaleDP returns. |
