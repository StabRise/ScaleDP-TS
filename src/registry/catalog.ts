...
export const STAGE_SPECS: readonly StageSpec[] = Object.freeze([
    ...
    },
    {
        ...
        defaults: asRecord(PADDLE_TEXT_DETECTOR_DEFAULTS),
        params: [
            ...baseParams({ input: { accepts: ['image'] } }),
            {
                key: 'preset',
    ...
    },
    {
        ...
        peer: 'ppu-paddle-ocr',
        cache: { kind: 'paddle-preset', param: 'preset' },
        defaults: asRecord(PADDLE_TEXT_RECOGNIZER_DEFAULTS),
        params: [
            ...baseParams({ input: { accepts: ['image'] } }),
            {
                key: 'preset',
    ...
    },
    {
        ...
        label: 'PaddleOCR on boxes',
        group: 'Recognise',
        subpath: '@stabrise/scaledp/ocr',
        summary:
            'Read exactly the regions a detector found with a PaddleOCR preset, straightening each one first.',
        consumes: ['image', 'boxes'],
        produces: 'document',
        peer: 'ppu-paddle-ocr',
        cache: { kind: 'paddle-preset', param: 'preset' },
        ...
        params: [
            ...
            lineTolerance,
            {
                ...
                help: 'Classify each crop 0°/180° and turn the inverted ones. Off by default: PaddleOCR turns tall crops itself, and only the 180° case needs this extra model.',
            },
            {
                key: 'onlyRotated',
                kind: 'boolean',
